...
int fftCore(cl_mem dst, cl_mem src, cl_mem spin, cl_int m, enum Mode direction){
	...
	ret = clReleaseEvent(bfly);
	ret = clReleaseEvent(brev);
	ret = clReleaseEvent(norm);
...
int main(){
	...
	program = clCreateProgramWithSource(context, 1, (const char**)&source_str, (const size_t*)&source_str, &ret);
	...
	setWorkSize(gws, lws, n, n);
	ret = clEnqueueNDRangeKernel(queue, trns, 2, NULL, gws, lws, 0, 0, NULL, NULL);
